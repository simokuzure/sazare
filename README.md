...
面向个人本地使用的日语练习项目，覆盖中译日短句、文章翻译和纯日语内容纠错。系统使用结构化 AI 输出完成题目生成、评分与纠错，并将用户确认或主动记录的复习内容接入学习闭环。
...
- 复习内容会生成中译日复习题，并通过复习周期和 SM-2 调度持续练习；卡片支持逻辑删除，删除后保留历史，再次添加时按全新卡片开始。
- 答题记录支持短句、文章和日语纠错筛选，以及列表/详情页内切换。
- 学习统计展示翻译与日语纠错的作答趋势、评分维度、复习重点和卡片状态；两类评分分开统计。
