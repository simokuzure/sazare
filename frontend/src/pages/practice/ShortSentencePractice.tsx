...
export default function ShortSentencePractice({ active = true }: { active?: boolean }) {
  ...
  async function handleConfirmErrors() {
    ...
    const confirmation = buildErrorConfirmations(answerReview.errorAnalysis, errorCandidates)
        ...
        <section className="surface answer-panel" aria-label={answerSubmitted ? text('评分结果', 'Score result') : text('答案输入', 'Answer input')}>
          ...
          {!answerSubmitted ? (
            ...
            <div className="answer-result">
              {answerNotice && (!answerReview || answerNotice.kind === 'error') ? <StatusNotice notice={answerNotice} /> : null}
