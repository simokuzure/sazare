  ...
  return <section className="page-content target-page review-page" aria-label={text('复习答题', 'Review answer')}>
    ...
    <section className="surface review-surface review-detail-surface">
      ...
      {detail ? <>
        ...
        {detail.reviewState === 'DERIVED_GENERATION_REQUIRED' ? <StateMessage title={text('需要生成衍生题', 'Generate a derived question')} message={detail.progress
          ? text(`本周期原题已通过，当前净成功 ${detail.progress.netSuccessCount} 次，尚未达到目标 ${detail.progress.targetSuccessCount} 次。生成一道衍生题继续积累净成功。`, `The original question passed. Current net successes: ${detail.progress.netSuccessCount}; target: ${detail.progress.targetSuccessCount}. Generate a derived question to continue.`)
          : text('本周期原题已通过，生成一道衍生题继续积累净成功。', 'The original question passed. Generate a derived question to continue.')} actionLabel={derivedGenerating ? text('生成中', 'Generating') : text('生成衍生题', 'Generate question')} actionDisabled={derivedGenerating} onAction={onGenerate} /> : null}
