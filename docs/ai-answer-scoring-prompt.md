...
请求可一次确认多条错误。`mode` 为 `NEW_USER_ERROR_TYPE` 时创建或复用用户错误类型；为 `EXISTING_USER_ERROR_TYPE` 时追加到已有用户错误类型。
...
服务端仅允许当前用户确认自己的 `REVIEWED` 作答，并从该作答填充 `user_id`、`question_id`。所有确认项在同一事务内写入 `user_answer_errors`；新建类型时同时写入 `user_error_types`。未确认的候选错误不会参与后续统计。
...
默认查询当前用户状态为 `ACTIVE` 的用户错误类型，供确认时选择已有类型。
