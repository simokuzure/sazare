...
用于生成日语翻译练习题。`learningMode` 决定中译日或英译日；下文中的中译日示例保留为默认兼容行为，英文模式由 `TranslationDirection` 统一派生英文题型与 Prompt 规则。
...
- 当前 AI 生成在保存前执行语义去重：使用 `sourceText + contextText` 的 768 维嵌入，与未逻辑删除的 AI、人工题及同批候选按余弦相似度比较；达到 `0.80` 的候选会被剔除并在最多三轮内补生成。`REVIEW_DERIVED` 不参与该流程。
...
`POST /api/questions/article-ai-generations` 使用独立的文章生成请求，其中 `lengthTier` 可选值及源文章长度如下：

| `lengthTier` | 中译日源文章 | 英译日源文章 |
| --- | --- | --- |
| `SHORT` | 60～100 个非空白字符 | 45～75 个英文单词 |
| `MEDIUM` | 120～180 个非空白字符 | 90～135 个英文单词 |
| `LONG` | 200～280 个非空白字符 | 150～210 个英文单词 |

- 未传或传空 `lengthTier` 时默认使用 `MEDIUM`。
- Prompt 必须写入所选档位的实际上下限；后端在保存前使用同一规则重新计数，越界时拒绝入库。
- JLPT 是词汇、语法和句式的能力上限，`difficulty` 是同一 JLPT 等级内的表达复杂度，不得突破所选等级。
- 主题、体裁、蓝图和补充要求决定文章内容；JLPT 与难度只决定表达方式，不得限制、替换、弱化或回避主题、情节、人物、冲突、观点或抽象议题。
- 源文章句法负担和日语参考译文都应符合所选语言难度；文章长度只约束源文章，不约束日语参考译文。
